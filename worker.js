export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health" && request.method === "GET") {
      return json({ version: "v16", database: Boolean(env.DB), uploads: Boolean(env.UPLOADS) });
    }
    if (url.pathname === "/api/uploads" && request.method === "POST") {
      ...
      return json({ url: `/admin/api/uploads/${encodeURIComponent(key)}` });
    }
    if (url.pathname.startsWith("/admin/api/uploads/") && request.method === "GET") {
      if (!env.UPLOADS) return new Response("Storage not connected", { status: 503 });
      const key = decodeURIComponent(url.pathname.slice("/admin/api/uploads/".length));
    ...
    if (url.pathname === "/api/briefs" || url.pathname === "/admin/api/briefs") {
      ...
      if (url.pathname === "/admin/api/briefs" && request.method === "GET") {
